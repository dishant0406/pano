...
const Detail = ({ title, description }: { title: string, description: any }) => {
    ...
    <div onMouseEnter={() => {
      ...
      <Image alt='bg'
        className={`w-full ${isHover ? 'opacity-100' : 'opacity-0'
          } absolute top-0 transition-all duration-300 left-0 z-[0] object-fill h-full`}
        height={1000} width={1000} src='/assets/images/sections/details.png' />
    ...
    <div className='w-full bg-black py-[5vh]'>
      {
        details.map((detail, i) => {
          return <Detail key={detail.title} title={detail.title} description={detail.description} />
        })
      }
    </div>
  )
